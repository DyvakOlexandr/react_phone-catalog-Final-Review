...
export const ProductDetails: React.FC<Props> = ({ category }) => {
  ...
  let selectedProduct: Phone | null = useSelector(
    (state: RootState): Phone | null =>
      state.phones.items.find(
        (phone: Phone) => String(phone.id) === String(itemId),
      ) ?? null,
  );

  const selectedTablet = useSelector((state: RootState) =>
    state.tablets.items.find(
      (tablet: Tablet) => String(tablet.id) === String(itemId),
    ),
  );

  const selectedAccess = useSelector((state: RootState) =>
    state.accessories.items.find(
      (acces: Accessories) => String(acces.id) === String(itemId),
    ),
  );
  ...
  const favouritesProductsFormatted = favouritesProducts.map(
  ...
  const productFromAll = products.find(
    (product: { itemId: string | undefined }) => product.itemId === itemId,
  );
  ...
  // Move the dispatch call into a useEffect hook
  useEffect(() => {
    if (productWithUpdatedPrice) {
      dispatch(setName(productWithUpdatedPrice.name));
    }
  }, [dispatch, productWithUpdatedPrice]); // Add dispatch and productWithUpdatedPrice to the dependency array
        ...
        <section className={'container'}>
          <div className={styles.productDetails}>
            <div className={styles.productDetails_navContainer}>
              ...
              <div className={styles.productDetails_navContainer_name}>
